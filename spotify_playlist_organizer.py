...
LLM_MODEL = os.getenv("OPENAI_MODEL", "gpt-3.5-turbo")  # Allow customizing the model via env var
...
def categorize_songs_with_llm(songs_batch, existing_categories=None):
    ...
    if existing_categories and len(existing_categories) > 0:
        ...
        if unique_cats:
            existing_categories_text = "Existing playlist categories (reuse these when appropriate):\n"
            existing_categories_text += ", ".join(unique_cats[:50])  # Limit to prevent token overflow
            existing_categories_text += "\n\n"
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if len(all_categories) >= len(songs_batch):
                ...
                logger.warning(f"LLM returned {len(all_categories)} category sets for {len(songs_batch)} songs. Retrying...")
...
def generate_summary(song_categories, all_songs):
    ...
    logger.info(f"Found {len(sorted_categories)} unique categories across {len(song_categories)} songs")
...
def main():
    ...
    logger.info(f"Found {len(uncategorized_songs)} uncategorized songs out of {len(all_songs)} total")
